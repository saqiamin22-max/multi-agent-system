...
with col_pipeline:
    ...
    step_card("02", "Reader Agent",  s("reader"), "Scrapes top URLs for deep content")
...
if st.session_state.running and not st.session_state.done:
    ...
    with st.spinner("🔍  Search Agent is working…"):
        ...
        sr = search_agent.invoke({
            "messages": [("user", topic_val)]   # ← Mistral style: bare topic
        })
        raw_search = sr["messages"][-1].content
        results["search"] = raw_search
        st.session_state.results = dict(results)

    # ── STEP 2: Reader — parse URLs then scrape each ────────────────────────
    with st.spinner("📄  Reader Agent is scraping top resources…"):
        reader_agent = build_reader_agent()

        # Safe parse: search agent returns a Python list of dicts
        try:
            url_results = ast.literal_eval(raw_search)
        except Exception:
            url_results = []

        scraped_data = []
        for item in url_results[:3]:                # top 3 URLs only
            url = item.get("url") if isinstance(item, dict) else None
            if not url:
                continue
            rr = reader_agent.invoke({
                "messages": [("user", url)]         # ← pass URL directly
            })
            scraped_data.append(rr["messages"][-1].content)
    ...
    with st.spinner("✍️  Writer is drafting the report…"):
        results["writer"] = writer_chain.invoke({
            "topic": topic_val,
            "research": results["reader"]           # ← only scraped content (Mistral style)
        })
        st.session_state.results = dict(results)
